<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="${comment}" prop="msg">
        <el-input
    ...
    <el-table v-loading="loading" :data="msgList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="${comment}" align="center" prop="msg" />
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="msgRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="${comment}" prop="msg">
          <el-input v-model="form.msg" placeholder="请输入${comment}" />
        </el-form-item>
        <el-form-item label="${comment}" prop="date">
          <el-date-picker clearable
            v-model="form.date"
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除sys_msg编号为"' + _ids + '"的数据项？').then(function() {
    return delMsg(_ids);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('system/msg/export', {
    ...queryParams.value
