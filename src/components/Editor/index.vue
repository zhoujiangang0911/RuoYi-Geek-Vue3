...
const emit = defineEmits(["on-change", 'on-text-change', 'on-selection-change', 'on-editor-change'])
onMounted(() => {
    init(0)
})
onBeforeUnmount(() => {
...
function init() {
    ...
    DataQuill.on("selection-change", (range, oldRange, source) => {
        emit("on-selection-change", range, oldRange, source);
    });
    DataQuill.on("editor-change", (eventName, ...args) => {
